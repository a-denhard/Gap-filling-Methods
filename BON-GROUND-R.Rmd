...
ground_dat = suppressWarnings(read.csv("D:/Work/Ground_measured_data/BON.csv"
                                      , header = TRUE, sep = ",")) 
...
# na = c(rep(1, 319), rep(2, 114), rep(3, 132), rep(4, 164), rep(5, 48), rep(6, 4), 9, 48)

n = c(319, 114, 132, 164, 48, 4, 1, 1) # first 7 col -- clear; next 7 col -- cloudy
m = matrix(c(1:30, 31:60, 61:90, 91:120, 121:150, 151:180, 271:301, 1441:1471), ncol = 8)
...
c(x, ratio, index) := f(x, n[8], m[,8], max_iter = 1000, ratio, index)
summary(x)
length(ratio)
```
...
k1 = detect_index(na.sorted$V1, function(x){x > 30})
 
k2 = detect_index(na.sorted$V1, function(x){x > 60})
 
k3 = detect_index(na.sorted$V1, function(x){x > 90})

k4 = detect_index(na.sorted$V1, function(x){x > 120})
...
MAE_p = cbind(rbind(round(MAE_p4.1, 1), round(MAE_p4.2, 1), round(MAE_p4.3, 1), round(MAE_p4.4, 1), round(MAE_p4.5, 1), round(MAE_p4.6, 1)), rbind(round(MAE_p6.1, 2), round(MAE_p6.2, 1), round(MAE_p6.3, 1), round(MAE_p6.4, 1), round(MAE_p6.5, 1), round(MAE_p6.6, 1)))
