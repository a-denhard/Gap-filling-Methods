...
ground_dat = suppressWarnings(read.csv("D:/Work/Ground_measured_data/SXF.csv"
                                      , header = TRUE, sep = ",")) 
...
# na = c(rep(1, 310), rep(2, 114), rep(3, 118), rep(4, 138), rep(5, 64), rep(6, 20), rep(7,4), 8, rep(9,2), 11, 48)

n = c(310, 114, 118, 138, 64, 20, 4, 1, 2, 1, 1) # first 7 col -- clear; next 7 col -- cloudy
m = matrix(c(1:30, 31:60, 61:90, 91:120, 121:150, 151:180, 181:210, 211:240, 241:270, 301:330, 1441:1470), ncol = 11)
...
c(x, ratio, index) :=  f(x, n[4], m[,4], max_iter = 1000, ratio, index)
summary(x)
length(ratio)

c(x, ratio, index) :=  f(x, n[5], m[,5], max_iter = 1000, ratio, index)
summary(x)
length(ratio)
...
k5 = detect_index(na.sorted$V1, function(x){x == 152})
# 
